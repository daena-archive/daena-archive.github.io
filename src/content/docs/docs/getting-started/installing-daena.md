...
description: Find the current alpha builds and verify release-specific requirements.
sidebar:
  order: 1
---

Daena is in active alpha development. Use the [latest GitHub release](https://github.com/daena-archive/daena/releases) as the authority for currently published operating-system builds, package formats, checksums, and known limitations.
...
- Keep a separate backup of any project you care about while Daena is alpha. Storage may change before `v1.0`.
...
The recommended path for most authors is the published alpha above. Building from source requires Deno, Rust, and the Tauri system dependencies, and is documented in `README.DEV.md` in the Daena repository.
...
<!-- TODO(screenshot): install step — Releases page with the current alpha assets list highlighted. Replacement: capture the latest release only; do not hard-code version numbers or platforms in the image caption. -->
